...
.controller('RoomCtrl', function ($rootScope, $scope, $location) {
    if($rootScope.pseudo){
        ...
        $scope.sendMessage = function(){
            if ($scope.input) {
                socket.emit('chatMessage', {"text":this.input,"pseudo":$rootScope.pseudo});
                $scope.input = '';
            }
        }
        ...
        // SOCKET IO
        var socket = io($rootScope.config.baseUrl, { query: "pseudo="+$rootScope.pseudo, reconnection: false});
